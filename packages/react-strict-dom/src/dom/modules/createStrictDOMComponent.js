...
export function createStrictDOMComponent<T: StrictHTMLElement, P: StrictProps>(
  ...
  const component: React.AbstractComponent<P, T> = React.forwardRef(
    function (props, forwardedRef) {
       ...
       * get directions
       */
      const componentDirection = props.dir || defaultProps?.dir;

      /**
       * get host props
       */
      const { style: defaultStyle, ...restDefaultProps } = defaultProps;
      const { for: htmlFor, style, ...restProps } = props;
      const hostProps: { ...P, htmlFor?: string } = {
        ...(restDefaultProps: $Rest<Partial<P>, $ReadOnly<{ style: mixed }>>),
        ...restProps
      };
      validateStrictProps(hostProps);

      if (componentDirection !== null) {
        hostProps.dir = componentDirection;
      }
      if (htmlFor != null) {
